...
const addGuiController = () => {\r
  ...
  const imageryLayer = viewer.imageryLayers.get(0)\r
  const gui = new dat.GUI()\r
  gui.add(controls, 'alpha', 0, 1).step(0.1).onChange(value => {\r
    imageryLayer.alpha = value\r
  })\r
  gui.add(controls, 'brightness', -1, 5).step(0.1).onChange(value => {\r
    imageryLayer.brightness = value\r
  })\r
  gui.add(controls, 'contrast', -1, 3).step(0.1).onChange(value => {\r
    imageryLayer.contrast = value\r
  })\r
  gui.add(controls, 'saturation', 0, 2).step(0.1).onChange(value => {\r
    imageryLayer.saturation = value\r
  })\r
}\r
\r
onMounted(() => {\r
